...
같은 명령을 한 번 더 실행해 Airflow 실행 두 건과 PostgreSQL 결과를 비교했습니다.

| 적재 run ID | 상태 | 입력 | 삽입 | 건너뜀 |
|---:|---|---:|---:|---:|
| 9 | `succeeded` | 3,223 | 0 | 3,223 |
| 10 | `succeeded` | 3,223 | 0 | 3,223 |

두 번 모두 Airflow 상태가 `success`였고 PostgreSQL의 최종 행 수는 실행 전후 모두
3,223행이었습니다. 따라서 같은 날짜가 반복 전달돼도 결과가 달라지거나 데이터가
늘어나지 않는 멱등성을 Airflow 경계에서도 확인했습니다.
